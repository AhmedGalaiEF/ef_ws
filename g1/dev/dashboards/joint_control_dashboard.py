...
out of scope, same as sdk_wrapper_v3.G1.move_ll_joint) as a live 3D skeleton
...
Two control paths, mirroring sdk_wrapper_v3.G1:
  ...
  * Dev mode — rt/lowcmd, all 29 joints, ai_sport stopped. Toggling it calls
    ...
    default gains sdk_wrapper_v3._LowCmd.write() uses, so nothing moves at
...
throughout sdk_wrapper_v3.py (LOWCMD_JOINTS = legs 0-11, waist 12-14,
arms 15-28).
...
# Make `from sdk_wrapper_v3 import G1` and `from hand_pose_navigation...`
# work regardless of cwd — mirrors sdk_wrapper_v3.py's own sys.path setup.
_DEV_DIR = Path(__file__).resolve().parent.parent
_G1_ROOT_DIR = _DEV_DIR.parent
for _p in (_DEV_DIR, _G1_ROOT_DIR):
...
# sdk_wrapper_v3.LOWCMD_JOINTS uses.
# ---------------------------------------------------------------------------
...
ENGAGE_DURATION_S = 1.0   # matches sdk_wrapper_v3.G1.engage_arms()
RELEASE_DURATION_S = 3.0  # matches sdk_wrapper_v3.G1.release_arms()
POLL_HZ = 20.0
PUBLISH_HZ = 50.0
...
# Same ArmIK defaults sdk_wrapper_v3.G1._arm_ik_solver() uses.
EE_IK_MAX_ITER = 24
...
class RobotLink:
    ...
    def connect(self) -> None:
        with self.lock:
            ...
            if self.g1 is not None:
                return
            try:
                from sdk_wrapper_v3 import G1  # deferred: only needed once connecting
    ...
    def toggle_dev_mode(self) -> tuple[bool, str]:
        # NOTE: sdk_wrapper_v3.G1.toggle_dev_mode() == toggle_service("ai_sport"),
