...
def serialize_lowstate(msg: LowState_):
    ...
    return {
        ...
        "bit_flag": bit_flag,
        "bit_flag_decoded": {
            "motor_timeout": bool(bit_flag & 0x80),
            "mcu_timeout": bool(bit_flag & 0x40),
            "remote_timeout": bool(bit_flag & 0x20),
            "battery_timeout": bool(bit_flag & 0x10),
            "auto_charging_not_charging": bool(bit_flag & 0x04),
            "board_current_error": bool(bit_flag & 0x02),
            "cmd_timeout": bool(bit_flag & 0x01),
        },
        "bms": {
            ...
            "bq_ntc": [int(v) for v in msg.bms_state.bq_ntc],
            "mcu_ntc": [int(v) for v in msg.bms_state.mcu_ntc],
            "cell_vol": [int(v) for v in msg.bms_state.cell_vol],
        },
        "motors": motors,
    }
...
def set_auto_charge(enable: bool):
    global low_gpio
    if enable:
        low_gpio &= 0xFE
    else:
...
def set_motor_power(enable: bool):
    global low_gpio
    if enable:
        low_gpio &= 0xFD
    else:
...
def low_level_loop():
    ...
    while True:
        if low_level_enabled and low_cmd_pub is not None:
            ...
            cmd.gpio = int(low_gpio) & 0xFF
            cmd.bms_cmd.off = 0xA5 if bms_off else 0x00
            cmd.crc = crc.Crc(cmd)
...
class UiServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/low/auto_charge":
            params = urllib.parse.parse_qs(parsed.query)
            state = params.get("state", ["0"])[0]
            set_auto_charge(state == "1")
            self._send_json({"ok": True, "gpio": int(low_gpio)})
            return
        if path == "/api/low/motor_power":
            params = urllib.parse.parse_qs(parsed.query)
            state = params.get("state", ["0"])[0]
            set_motor_power(state == "1")
            self._send_json({"ok": True, "gpio": int(low_gpio)})
            return
        if path == "/api/low/bms_off":
            params = urllib.parse.parse_qs(parsed.query)
            state = params.get("state", ["0"])[0]
            set_bms_off(state == "1")
            self._send_json({"ok": True, "bms_off": bool(bms_off)})
            return
...
<body>
  ...
  <div class="grid">
    ...
    <div class="panel">
      ...
      <div class="row">
        ...
        <div class="kv">
          <span>Motor Order</span>
          <strong>FR_0..2, FL_0..2, RR_0..2, RL_0..2 (0-11)</strong>
        </div>
        <div class="kv">
          <span>Stream</span>
          <div class="controls">
        ...
        <div class="kv">
          <span>LowCmd Power/Charge</span>
          <div class="controls">
            <button onclick="autoCharge(true)" class="primary">Auto Charge ON</button>
            <button onclick="autoCharge(false)" class="warn">Auto Charge OFF</button>
            <button onclick="motorPower(true)" class="primary">Motor Power ON</button>
            <button onclick="motorPower(false)" class="warn">Motor Power OFF</button>
            <button onclick="bmsOff(true)" class="warn">Battery OFF (0xA5)</button>
            <button onclick="bmsOff(false)">Battery ON</button>
          </div>
        </div>
      </div>
    ...
    <div class="panel">
      <h2>Odometer (Legged Fusion)</h2>
      <div class="row">
        <div class="kv"><span>Source</span><strong>rt/sportmodestate.position</strong></div>
        <div class="kv"><span>Position</span><strong id="odo_pos">--</strong></div>
        <div class="kv"><span>Velocity</span><strong id="odo_vel">--</strong></div>
        <div class="kv"><span>Yaw Speed</span><strong id="odo_yaw">--</strong></div>
      </div>
    </div>
    <div class="panel">
      <h2>Sport State</h2>
    ...
    <div class="panel">
      ...
      <div class="row" style="margin-top:12px;">
        <div class="kv"><span>IMU Quat</span><strong id="imu_quat">--</strong></div>
        <div class="kv"><span>IMU Temp</span><strong id="imu_temp">--</strong></div>
        <div class="kv"><span>ADC Reel</span><strong id="adc_reel">--</strong></div>
        <div class="kv"><span>Ntc1/Ntc2</span><strong id="ntc_temp">--</strong></div>
        <div class="kv"><span>Fan RPM</span><strong id="fan_rpm">--</strong></div>
      </div>
      <div class="row" style="margin-top:12px;">
        <div class="kv"><span>Bit Flag</span><strong id="bit_flag">--</strong></div>
        <div class="kv"><span>Timeouts</span><strong id="timeouts">--</strong></div>
        <div class="kv"><span>Battery SOC</span><strong id="bms_soc">--</strong></div>
        <div class="kv"><span>BMS Current</span><strong id="bms_current">--</strong></div>
        <div class="kv"><span>BMS Cycle</span><strong id="bms_cycle">--</strong></div>
        <div class="kv"><span>BMS Temps</span><strong id="bms_temps">--</strong></div>
      </div>
  ...
  <script>
    ...
    function autoCharge(enable) {{
      fetch(`/api/low/auto_charge?state=${{enable ? 1 : 0}}`)
        .then(r => r.json())
        .then(d => document.getElementById('cmd_result').textContent = JSON.stringify(d));
    }}

    function motorPower(enable) {{
      fetch(`/api/low/motor_power?state=${{enable ? 1 : 0}}`)
        .then(r => r.json())
        .then(d => document.getElementById('cmd_result').textContent = JSON.stringify(d));
    }}

    function bmsOff(enable) {{
      fetch(`/api/low/bms_off?state=${{enable ? 1 : 0}}`)
        .then(r => r.json())
        .then(d => document.getElementById('cmd_result').textContent = JSON.stringify(d));
    }}
    ...
    function update() {{
      fetch('/api/state')
        ...
        .then(data => {{
          ...
          document.getElementById('odo_pos').textContent = s.position ? s.position.map(v => v.toFixed(2)).join(', ') : '--';
          document.getElementById('odo_vel').textContent = s.velocity ? s.velocity.map(v => v.toFixed(2)).join(', ') : '--';
          document.getElementById('odo_yaw').textContent = s.yaw_speed ?? '--';
          ...
          document.getElementById('imu_quat').textContent = l.imu_quat ? l.imu_quat.map(v => v.toFixed(2)).join(', ') : '--';
          document.getElementById('imu_temp').textContent = l.imu_temp ?? '--';
          document.getElementById('adc_reel').textContent = l.adc_reel ?? '--';
          document.getElementById('ntc_temp').textContent = (l.temperature_ntc1 !== undefined) ? `${{l.temperature_ntc1}} / ${{l.temperature_ntc2}}` : '--';
          document.getElementById('fan_rpm').textContent = l.fan_frequency ? l.fan_frequency.join(', ') : '--';
          document.getElementById('bit_flag').textContent = l.bit_flag ?? '--';
          if (l.bit_flag_decoded) {{
            const bd = l.bit_flag_decoded;
            document.getElementById('timeouts').textContent =
              `motor:${{bd.motor_timeout}} mcu:${{bd.mcu_timeout}} remote:${{bd.remote_timeout}} batt:${{bd.battery_timeout}}`;
          }} else {{
            document.getElementById('timeouts').textContent = '--';
          }}
          if (l.bms) {{
            document.getElementById('bms_soc').textContent = `${{l.bms.soc}}%`;
            document.getElementById('bms_current').textContent = l.bms.current ?? '--';
            document.getElementById('bms_cycle').textContent = l.bms.cycle ?? '--';
            document.getElementById('bms_temps').textContent = `${{(l.bms.bq_ntc || []).join(', ')}} / ${{(l.bms.mcu_ntc || []).join(', ')}}`;
          }} else {{
            document.getElementById('bms_soc').textContent = '--';
            document.getElementById('bms_current').textContent = '--';
            document.getElementById('bms_cycle').textContent = '--';
            document.getElementById('bms_temps').textContent = '--';
          }}
